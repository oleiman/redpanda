...
template<size_t... Is>
per_group<min_share_group_state> make_group_states(std::index_sequence<Is...>) {
    return {{min_share_group_state{
      static_cast<group_id>(Is),
      fmt::format(
        "cloud_io/min_share_policy/reserved/{}",
        to_string_view(static_cast<group_id>(Is)))}...}};
}

} // namespace

fmt::iterator min_share_group_state::format_to(fmt::iterator out) const {
...
min_share_policy::min_share_policy(size_t capacity, min_share_policy_config cfg)
  : scheduler_policy(capacity)
  , _current_total_capacity(capacity)
  , _shared(0, "cloud_io/min_share_policy/shared")
  ...
  , _now_fn([] { return ss::lowres_clock::now(); }) {
    ...
    vassert(
      target_sum <= capacity,
      "min_share_policy: target_reserved sum ({}) exceeds capacity ({})",
      target_sum,
      capacity);
...
min_share_policy::~min_share_policy() noexcept {
    for (const auto& gs : _groups) {
        vassert(
          gs.waiters.empty(),
          "cloud_io::min_share_policy destroyed with active waiters");
    }
}

ss::future<> min_share_policy::stop() {
...
size_t min_share_policy::in_flight(group_id g) const noexcept {
    return _groups[g].in_flight;
}

size_t min_share_policy::waiters(group_id g) const noexcept {
    return _groups[g].waiter_count();
}

size_t min_share_policy::available_slots() const noexcept {
...
size_t min_share_policy::total_capacity() const noexcept {
    return _current_total_capacity;
}

ss::future<> min_share_policy::admit(group_id g, ss::abort_source& as) {
    // Fast path.
    if (try_admit(g)) {
        co_return;
    }
...
bool min_share_policy::try_admit(group_id g) noexcept {
    auto& gs = _groups[g];
...
void min_share_policy::release(group_id g) noexcept {
    auto& gs = _groups[g];
...
bool min_share_policy::dispatch_next() noexcept {
    ...
    if (_dispatch_counter++ % 1000 == 0) {
        vlog(
          log.debug,
          "min_share_policy: dispatch #{} picked={} | {}",
          _dispatch_counter,
...
void min_share_policy::set_total_slots(size_t desired) {
    ...
    vlog(
      log.info,
      "cloud_io min_share_policy total slots: {} -> {}",
      _current_total_capacity,
      desired);
    _current_total_capacity = desired;
}

void min_share_policy::set_target_reserved(group_id g, size_t value) {
    auto& gs = _groups[g];
...
size_t min_share_policy::target_reserved(group_id g) const noexcept {
    return _groups[g].target_reserved;
}

size_t min_share_policy::current_reserved(group_id g) const noexcept {
    return _groups[g].current_reserved();
}

uint64_t min_share_policy::admit_total(group_id g) const noexcept {
    return _groups[g].admit_total;
}

uint64_t min_share_policy::admit_immediate_total(group_id g) const noexcept {
    return _groups[g].admit_immediate_total;
}

size_t min_share_policy::total_waiters() const noexcept {
...
void min_share_policy::set_now_fn_for_test(now_fn_t fn) {
    _now_fn = std::move(fn);
}

void min_share_policy::reclaim_idle_reservations(
...
std::optional<group_id> min_share_policy::pick_refill_candidate() noexcept {
