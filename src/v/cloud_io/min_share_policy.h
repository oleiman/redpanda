...
struct min_share_policy_config {
    per_group<uint32_t> target_reserved{};

    min_share_policy_config() = default;
    min_share_policy_config(
...
/// the group's reserved_sem inside min_share_group_state) or the common
...
/// the dwell window. See min_share_group_state for the mechanism details.
///
/// Per-shard; not movable (metrics lambdas capture `this`).
class min_share_policy final : public scheduler_policy {
public:
    explicit min_share_policy(size_t capacity, min_share_policy_config = {});
    min_share_policy(const min_share_policy&) = delete;
    min_share_policy& operator=(const min_share_policy&) = delete;
    min_share_policy(min_share_policy&&) = delete;
    min_share_policy& operator=(min_share_policy&&) = delete;
    ~min_share_policy() noexcept override;
    ...
    // ---- min_share_policy-specific public API (not on ABC) ----

    /// Runtime target_reserved mutator. Construction installs initial
    /// targets via min_share_policy_config; this exists for runtime
    ...
    /// min_share_group_state::current_reserved for semantics.
...
private:
    ...
    /// min_share_group_state for the layout and invariants.
    per_group<min_share_group_state> _groups;
