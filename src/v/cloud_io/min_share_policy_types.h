...
///
///   - A single common pool of slots (min_share_policy::_shared) any group
...
struct min_share_group_state {
    min_share_group_state(group_id id, ss::sstring sem_name)
      : id(id)
      , reserved_sem(0, std::move(sem_name)) {}

    min_share_group_state(const min_share_group_state&) = delete;
    min_share_group_state& operator=(const min_share_group_state&) = delete;
    min_share_group_state(min_share_group_state&&) = delete;
    min_share_group_state& operator=(min_share_group_state&&) = delete;
    ~min_share_group_state() = default;
    ...
    intrusive_list<min_share_waiter, &min_share_waiter::link> waiters;
    ...
    /// nullopt before the group's first active interval, and reset to
    /// nullopt when the policy reclaims this group's lane after dwell.
    /// Gates reclamation and refill eligibility.
    ...
    /// True if the group has been idle longer than default_dwell_duration
    /// and is eligible for reclamation.
    bool is_dwell_expired(ss::lowres_clock::time_point now) const noexcept {
        if (is_active() || !inactive_since.has_value()) {
            return false;
        }
        return now - *inactive_since >= default_dwell_duration;
    }
    ...
    void enqueue_waiter(min_share_waiter& w, uint64_t seq) noexcept {
        w.seq = seq;
        waiters.push_back(w);
    }
