...
> Status: design accompanying the `reservation_policy` PR. Not a
...
The current policy is `reservation_policy`: a two-tier slot
...
(passthrough_policy baseline, `ctflex-20260519-164636`):
...
|---|---|---|
| `cloud_io_scheduler_policy` | `passthrough` | `passthrough` disables admission; `reservation` enables. |
| `cloud_io_scheduler_reservation` | 2 per group | List of `"group_name:slots"` entries. Unknown names ignored with warn. |
...
| Metric | passthrough baseline | reservation alone |
|---|---|---|
...
| Metric | reservation (2/2/2) | all-reserved (6/7/7) |
|---|---|---|
...
Kept available as `cloud_io_scheduler_policy=passthrough` for
