...
/// Built from cluster config at scheduler construction time, or
/// directly in tests. Default-constructed is all zeros; the
/// initializer-list constructor takes `{group_id, value}` pairs and
/// leaves unmentioned groups at zero.
...
struct scheduler_config {
    ...
    /// Reservation policy targets. Consumed by policies that implement
    /// reservation semantics; ignored by passthrough. Absent => all
    /// targets are zero (no reserved lanes, common-pool-only behavior
    /// when a reservation policy is selected).
