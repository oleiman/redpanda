...
void configure(
  cloud_io::min_share_policy& policy,
...
TEST_CORO(MinSharePolicyTest, BasicAdmitReleaseAndAccounting) {
    cloud_io::min_share_policy policy{4};
...
TEST_CORO(MinSharePolicyTest, SaturationCausesQueueing) {
    cloud_io::min_share_policy policy{2};
    configure(policy, 2);
    ss::abort_source as;
...
TEST_CORO(MinSharePolicyTest, AbortCancelsQueuedWait) {
    cloud_io::min_share_policy policy{1};
    configure(policy, 1);
    ss::abort_source as;
...
TEST_CORO(MinSharePolicyTest, AlreadyAbortedSourceRejectsSlowPath) {
    cloud_io::min_share_policy policy{1};
    configure(policy, 1);
    ss::abort_source as;
...
TEST_CORO(MinSharePolicyTest, CrossGroupDispatchWakesQueuedWaiter) {
    cloud_io::min_share_policy policy{1};
    configure(policy, 1);
    ss::abort_source as;
...
TEST_CORO(MinSharePolicyTest, MultiGroupFillsCapacity) {
    cloud_io::min_share_policy policy{4};
    configure(policy, /*total_slots=*/4);
    ss::abort_source as;
...
TEST_CORO(MinSharePolicyTest, ReservationLaneSemantics) {
    // capacity=6, pu_min=2 -> _shared=4, _reserved[pu]=2.
    // Fill shared with cf; verify pu still admits via reserved while
    // cf cannot consume pu's reserved slots. Then queue a cf waiter
    // and verify the first pu releases (reserved) do NOT dispatch cf;
    // the slot returns to pu's reserved pool. Only the shared release
    // dispatches cf.
    cloud_io::min_share_policy policy{6};
...
TEST_CORO(MinSharePolicyTest, ReservedAndSharedSlotsCoexist) {
    cloud_io::min_share_policy policy{6};
    configure(policy, /*total_slots=*/6, /*target_reserved=*/{2, 0, 0});
    ss::abort_source as;
...
TEST_CORO(MinSharePolicyTest, ReclaimsIdleReservationToCommonPool) {
    cloud_io::min_share_policy policy{6};
    configure(policy, /*total_slots=*/6, /*target_reserved=*/{2, 0, 0});
...
TEST_CORO(MinSharePolicyTest, WorkConservingCycleReclaimThenRefill) {
    cloud_io::min_share_policy policy{6};
    configure(policy, /*total_slots=*/6, /*target_reserved=*/{2, 0, 0});
...
TEST_CORO(MinSharePolicyTest, RefillOnlyFiresAfterDispatchNext) {
    cloud_io::min_share_policy policy{4};
...
TEST_CORO(MinSharePolicyTest, DispatchAndRefillAcrossUnderTargetGroups) {
    cloud_io::min_share_policy policy{6};
