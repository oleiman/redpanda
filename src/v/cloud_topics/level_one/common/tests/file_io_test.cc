...
TEST_F_CORO(file_io_test_fixture, MergedReadHitsCacheAfterSuccess) {
    ...
    // Synthesize an in-flight download by becoming the leader for
    // br_key; the leader's destruction publishes the outcome to
    // merged readers.
    ss::abort_source leader_as;
    auto join = inflight_downloads().join_or_lead(br_key, leader_as);
    ASSERT_TRUE_CORO(
      std::holds_alternative<inflight_download_map::leader_guard>(join));
    std::optional<inflight_download_map::leader_guard> leader;
    leader.emplace(
      std::get<inflight_download_map::leader_guard>(std::move(join)));
    ...
    // Stand in for the in-flight download: warm the cache, then drop
    // the leader so the dtor publishes nullopt to the merged reader.
...
TEST_F_CORO(file_io_test_fixture, MergedReadAborted) {
    ...
    // The leader's dtor publishes + erases when this scope ends, so
    // the synthesized entry doesn't outlive the fixture.
}
