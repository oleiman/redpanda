...
// in-flight waiter path without touching object storage. The cold-miss
// leader path is bench-validated, not unit-tested here.
...
// Waiter sees an in-flight download for its (oid, pos, size), awaits
// the leader's promise, and on resolve returns the cached bytes.
TEST_F_CORO(file_io_test_fixture, ByteRangeWaiterResolvesOnLeaderSuccess) {
    auto id = create_object_id();
    constexpr size_t pos = 100;
    constexpr size_t size = 256;
    auto br_key = byte_range_key(id, pos, size);

    // Synthesize the leader by inserting into the in-flight map before
    // the waiter starts; we'll resolve the promise by hand below.
    ...
    // Stand in for the leader: warm the cache, then resolve.
...
// Waiter sees an in-flight download whose leader fails; waiter must
// propagate the leader's errc rather than racing on a fresh attempt.
TEST_F_CORO(file_io_test_fixture, ByteRangeWaiterPropagatesLeaderError) {
    auto id = create_object_id();
    constexpr size_t pos = 100;
    constexpr size_t size = 256;
    auto br_key = byte_range_key(id, pos, size);
    ...
    // Leader failed downloading; waiters propagate the same errc.
    co_await ss::sleep(5ms);
...
// Aborting a waiter while it's suspended on the leader's promise must
// surface as cloud_op_timeout (the abort_source-aware contract on the
// shared_future), not hang.
TEST_F_CORO(file_io_test_fixture, ByteRangeWaiterAborted) {
    auto id = create_object_id();
    constexpr size_t pos = 100;
    constexpr size_t size = 256;
    auto br_key = byte_range_key(id, pos, size);
