...
// the hierarchical lookup (prefetch cache hit, byte-range cache hit,
// in-flight waiter paths) without touching object storage. Step 6 (cold
// miss leader) is bench-validated, not unit-tested here.
...
public:
    ...
    void put_prefetch(
      const object_id& id, size_t seg_pos, size_t seg_size, char fill_char) {
        auto key = prefetch_key(id, seg_pos, seg_size);
...
constexpr auto cf = cloud_io::group_id::consumer_fetch;

} // namespace

// Step 2: prefetch cache hit serves a slice via get_stream_range at the
// segment's interior offset.
TEST_F_CORO(file_io_test_fixture, Step2PrefetchCacheHit) {
    ...
    ss::abort_source as;
    auto stream_result = co_await io().read_object(extent, &as, cf);
...
// Step 2 takes precedence over step 3 when both files exist.
...
// Step 4: in-flight prefetch waiter resolves on success; on retry the
// step-2 cache lookup hits the now-populated prefetch file.
TEST_F_CORO(file_io_test_fixture, Step4PrefetchWaiterResolvesSuccess) {
    ...
    ss::abort_source as;
    auto fut = io().read_object(extent, &as, cf);
...
// Step 4: prefetch waiter sees the leader fail; falls through to the
// byte-range path which serves the consumer's bytes.
TEST_F_CORO(
...
// Step 5: byte-range waiter joins an in-flight download, awaits the
// shared promise, then retries and reads the cached bytes.
...
// Step 5: byte-range waiter propagates the leader's errc rather than
// racing on a fresh attempt.
TEST_F_CORO(file_io_test_fixture, Step5ByteRangeWaiterResolvesError) {
    auto id = create_object_id();
    constexpr size_t pos = 100;
    constexpr size_t size = 256;
    auto br_key = byte_range_key(id, pos, size);
...
// Aborting a prefetch waiter while it's suspended on the shared
// promise surfaces as cloud_op_timeout (abort_source-aware contract on
// the shared_future), not a hang.
TEST_F_CORO(file_io_test_fixture, Step4PrefetchWaiterAborted) {
    auto id = create_object_id();
    constexpr size_t seg_size = 4096;
    auto pf_key = prefetch_key(id, /*seg_pos=*/0, seg_size);
    ...
    // Clean up so the synthesized leader entry doesn't outlive the
    // fixture.
    pf_it->second.set_value(std::nullopt);
    inflight_prefetches().erase(pf_it);
}

// Aborting a byte-range waiter while it's suspended on the shared
// promise must surface as cloud_op_timeout, not hang.
TEST_F_CORO(file_io_test_fixture, Step5ByteRangeWaiterAborted) {
    auto id = create_object_id();
    constexpr size_t pos = 100;
    constexpr size_t size = 256;
    auto br_key = byte_range_key(id, pos, size);
