...
public:
    ...
    /// `oid`, it is replaced. The cache wraps `footer_value` in a
    /// `lw_shared_ptr` and returns it so the caller can reuse it. When the
    /// cache is disabled, the wrap still happens but the entry is not stored.
    footer_ptr put(object_id oid, footer footer_value);

    /// True when `target_max_size > 0`. When false, callers should skip
    /// `get`/`put` so we don't pollute metrics by missing through a disabled
    /// cache
