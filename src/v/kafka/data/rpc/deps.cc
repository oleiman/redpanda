...
public:
    ...
    ss::future<> wait_for_term_change(
      ...
      ss::abort_source& as) final {
        ...
        auto promise = ss::make_lw_shared<expiring_promise<void>>();
        auto ntp = model::ntp(tp_ns.ns, tp_ns.tp, pid);
        auto n_id = _table->local().register_leadership_change_notification(
          ntp, [promise](const model::ntp&, model::term_id, model::node_id) {
              promise->set_value();
          });
        ...
        try {
            ...
            // Abort source fired during shutdown; the retry loop's own
            // abort-source check will bail on the next iteration.
        }
    }
