...
public:
    ...
    /// service until \p deadline. \p stale_term is the term that was
    /// current when the caller looked up the leader. If the table has
    /// already advanced past this term, returns immediately instead of
    /// blocking.
    virtual ss::future<> wait_for_term_change(
