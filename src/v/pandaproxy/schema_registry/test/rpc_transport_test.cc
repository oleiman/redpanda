...
TEST_P(SchemaRegistryRpcTransportTest, GetHighWatermarkEmpty) {
...
TEST_P(SchemaRegistryRpcTransportTest, ConsumeRangeProducedBatches) {
    ...
    transport()
      .consume_range(
        model::offset(0),
        model::offset(3),
        [&records_consumed](model::record_batch b) -> ss::future<> {
...
TEST_P(SchemaRegistryRpcTransportTest, ConsumeRangePastHwmThrows) {
    // Produce one batch at offset 0, then ask consume_range to read past the
    // actual HWM. The first iteration succeeds; the second finds no records
    // and the transport surfaces a kafka::exception.
    transport().produce(make_batch("k1", "v1")).get();
    auto hwm = transport().get_high_watermark().get();
    ...
    EXPECT_THROW(
      transport()
        .consume_range(
          model::offset(0),
          hwm + model::offset(10),
          [](model::record_batch) -> ss::future<> { return ss::now(); })
        .get(),
      kafka::exception);
}
