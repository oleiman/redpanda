...
TEST_F(WasmCacheTest, CachesEngines) {
    ...
    invoke_on_all([&factory] {
        auto engine_one = factory->make_engine(nullptr);
        auto engine_two = factory->make_engine(nullptr);
    ...
    auto engine = factory->make_engine(nullptr).get();
    EXPECT_EQ(state()->engines, ss::smp::count);
    engine = nullptr;
    EXPECT_EQ(state()->engines, ss::smp::count);
...
TEST_F(WasmCacheTest, CanMultiplexEngines) {
    auto meta = random_metadata();
    auto factory = ss::make_foreign(make_factory(meta));
    auto engine_one = factory->make_engine(nullptr).get();
    auto engine_two = factory->make_engine(nullptr).get();
    auto engine_three = factory->make_engine(nullptr).get();
...
TEST_F(WasmCacheTest, GC) {
    ...
    invoke_on_all([&factory] { factory->make_engine(nullptr).get(); });
    EXPECT_EQ(state()->engines, 0);
...
TEST_F(WasmCacheTest, EngineReplacementBeforeGC) {
    ...
    // replaced in the cache instead of being inserted.
    engine = factory->make_engine(nullptr).get();
    EXPECT_EQ(state()->engines, 1);
}
