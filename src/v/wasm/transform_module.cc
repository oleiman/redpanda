...
transform_module::read_batch_metadata(int32_t key, ffi::array<uint8_t> buf) {
    ...
    case metadata_key::client_port: {
        constexpr size_t port_size = 2;
        if (buf.size() < port_size) {
            co_return INVALID_BUFFER;
        }
        buf[0] = static_cast<uint8_t>(info.client_port >> 8);
        buf[1] = static_cast<uint8_t>(info.client_port & 0xFF);
        co_return static_cast<int32_t>(port_size);
    }
    case metadata_key::tls_enabled: {
        constexpr size_t tls_size = 1;
        if (buf.size() < tls_size) {
            co_return INVALID_BUFFER;
        }
        buf[0] = info.tls_enabled ? uint8_t{1} : uint8_t{0};
        co_return static_cast<int32_t>(tls_size);
    }
    }
    co_return 0;
}
